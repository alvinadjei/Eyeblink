...
class VideoWindow(QMainWindow):
    def __init__(self):
        ...
        self.df_stim = pd.DataFrame(columns=['Trial #', 'CS Timestamp'])  # Create df to hold CS timestamps for each trial
        ...
        if event.key() == Qt.Key_Space:  # Spacebar pressed
            if self.ellipse_start and self.ellipse_end:  # Check if ellipse is drawn
                if not self.running:  # Only start a new thread if not already running
                    ...
                    threading.Thread(target=self.test__save_current_fec).start()  # Save images during trial
                    threading.Thread(target=self.test__stimuli).start()  # Send stimulif
                else:
                    self.running = False
    ...
    def test__save_current_fec(self):        
        ...
        while self.running:
            if self.frame is not None and self.trial_in_progress:
                ...
                # Add row containing current timestamp, trial number, and FEC to df
    ...
    def test__stimuli(self):
        ...
        for i in range(num_trials):
            ...
            try:
                ...
                self.df_stim.loc[len(self.df_stim)] = [self.trial_ind, cs_timestamp]  # Add CS timestamp to df_stim
